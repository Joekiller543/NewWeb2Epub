...
export const useNovelStore = create((set, get) => ({
  ...
  initSocket: () => {
    ...
    // Use environment variable for API URL or default to relative path for dev proxy
  ...
  analyzeNovel: async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/novel-info`, {
        method: 'POST',
  ...
  fetchChapters: async () => {
    ...
    const fetchBatch = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/api/chapters-batch`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
